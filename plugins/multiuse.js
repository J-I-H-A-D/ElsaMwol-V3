...
Asena.addCommand({ pattern: 'insta ?(.*)', fromMe: false, desc: IG_DESC }, async (message, match) => {
    ...
    if (type === 'image') { await message.sendMessage(data, MessageType.image, { caption: "Made By ElsaMwolV3" }) }
    else if (type === 'video') { await message.sendMessage(data, MessageType.video, { caption: "Made By ElsaMwolV3" }) }
});
//const axios = require('axios')
